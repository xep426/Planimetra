...
export function drawDoors(ctx: CanvasRenderingContext2D, dc: DrawContext) {
  ...
  doors.forEach(door => {
    ...
    if (placement.draw) {
      ...
      ctx.fillStyle = isSelected ? '#9a3412' : (isDoorMode ? '#1e40af' : defaultLabelBg);
      ctx.strokeStyle = isSelected ? '#ea580c' : (isDoorMode ? '#3b82f6' : defaultLabelBorder);
      ...
      ctx.fillStyle = isSelected ? '#fbbf24' : (isDoorMode ? '#9ca3af' : defaultLabelText);
