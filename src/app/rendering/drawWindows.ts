...
export function drawWindows(ctx: CanvasRenderingContext2D, dc: DrawContext) {
  ...
  windows.forEach(win => {
    ...
    if (placement.draw) {
      ...
      ctx.fillStyle = isSelected ? '#9a3412' : (isWindowMode ? '#1e40af' : defaultLabelBg);
      ctx.strokeStyle = isSelected ? '#ea580c' : (isWindowMode ? '#3b82f6' : defaultLabelBorder);
      ...
      ctx.fillStyle = isSelected ? '#fbbf24' : (isWindowMode ? '#9ca3af' : defaultLabelText);
